...
* Jakarta Transactions
    ** == 💡local Java interfaces BETWEEN 💡
        *** transaction manager -- & -- parties involved | distributed transaction system (application, the resource manager, application server)
    ** ==
        *** high-level application interface /
            **** allows
                ***** define the transactional application's boundaries
            **** uses
                ***** applications developers
        *** high-level transaction manager interface /
            **** allows
                ***** an application server can control the transaction boundary
            **** uses
...
* `jakarta.transaction.Transactional` annotation
    ** enables
        *** 💡the application can DECLARATIVELY control transaction boundaries | Jakarta CDI managed beans💡 |
            **** class
            **** method
                ***** HIGHER PRIORITY
    ** https://github.com/dancer1325/jakartaee-enterprise-beans/blob/master/spec/src/main/asciidoc/core/SupportForTransactions.adoc[Jakarta Enterprise Beans specification's support transactions]
        *** -- via -- implementation of Jakarta CDI interceptors /
            **** interposes |
                ***** business method invocations ONLY
    ** 's arguments
        *** `TxType` element
...
or `NEVER`, an `IllegalStateException` must be thrown
